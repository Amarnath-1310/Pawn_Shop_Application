...
import { createCustomerHandler, deleteCustomerHandler, listCustomersHandler, updateCustomerHandler, } from './handlers/customers.js';
import { createLoanHandler, getLoanHandler, listLoansHandler, updateLoanStatusHandler, } from './handlers/loans.js';
import { createRepaymentHandler, listRepaymentsHandler, syncLoanStatusHandler, } from './handlers/repayments.js';
...
const mapRequestToEvent = (method, path, body, pathParameters) => ({
    version: '2.0',
...
const server = createServer((req, res) => {
    ...
    const pathParameters = match
        ? matchedRoute.paramNames.reduce((acc, param, index) => {
    ...
    req
        ...
        try {
            ...
            const raw = await matchedRoute.handler(event, {}, () => undefined);
            const response = (() => {
                if (!raw) {
                    return {
                        statusCode: 204,
